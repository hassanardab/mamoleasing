...
class _AddEventDialogState extends State<AddEventDialog> {
  ...
  void initState() {
    ...
    _customerEmailController = TextEditingController(text: event?.customerEmail);
    _customerPhoneController = TextEditingController(text: event?.customerPhone);
    _amountController = TextEditingController(text: event?.amount.toString());
    _paidAmountController = TextEditingController(text: event?.paidAmount?.toString() ?? '0');
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _buildTextField(_customerNameController, 'Customer Name', validator: (v) => v!.isEmpty ? 'Required' : null),
              ...
              _buildTextField(_amountController, 'Amount', keyboardType: TextInputType.number, validator: (v) => v!.isEmpty ? 'Required' : null),
              _buildTextField(_paidAmountController, 'Paid Amount', keyboardType: TextInputType.number),
              const SizedBox(height: 16),
              _buildSectionTitle(context, 'Location & Status'),
              _buildTextField(_placeController, 'Place (Optional)'),
              _buildStatusDropdown(),
              const SizedBox(height: 16),
              _buildDateTimePicker(context, 'Start', _startDate, (date) => setState(() => _startDate = date)),
              _buildDateTimePicker(context, 'End', _endDate, (date) => setState(() => _endDate = date)),
            ],
          ),
        ),
      ),
    ...
    return Padding(
      ...
      child: DropdownButtonFormField<BookingStatus>(
        ...
        items: BookingStatus.values.map((status) {
          return DropdownMenuItem<BookingStatus>(value: status, child: Text(status.name));
        }).toList(),
  ...
  Widget _buildDateTimePicker(BuildContext context, String label, DateTime initialDate, Function(DateTime) onDateChanged) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 6.0),
      child: Row(
        children: [
          Expanded(child: Text('$label: ${DateFormat.yMd().add_jm().format(initialDate)}')),
          TextButton(
            onPressed: () async {
              final date = await showDatePicker(context: context, initialDate: initialDate, firstDate: DateTime(2000), lastDate: DateTime(2101));
              if (date == null) return;
  ...
  Future<void> _saveEvent() async {
    ...
    if (companyId == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error: Company not selected.'), backgroundColor: Colors.red));
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to save event: $e'), backgroundColor: Colors.red));
    }
  }
}
